...
function filtraTarefas() {
 ...
 let tarefasFinalizadas = tarefas.filter(item => item.status === "done")
 let nomeTarefasFinalizadas = tarefasFinalizadas.map(item => item.titulo)
 return nomeTarefasFinalizadas
}
```